  ...
  return <header className={cn('fixed w-full top-0 z-50 transition-all duration-300', isScrolled ? 'glass py-3' : 'bg-transparent py-6')}>
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-4">
          <a href={`tel:${siteConfig.contactInfo.phone.replace(/\s+/g, "")}`} className="flex items-center gap-1 text-foreground/80 hover:text-foreground transition-colors duration-300">
            <Phone size={16} />
      ...
      <div className={cn('fixed inset-0 bg-background pt-20 px-4 z-40 transition-all duration-300 ease-in-out md:hidden', isMenuOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full pointer-events-none')}>
        ...
        <nav className="flex flex-col space-y-6">
          ...
          <a href={`tel:${siteConfig.contactInfo.phone.replace(/\s+/g, "")}`} className="text-xl font-medium py-2 border-b border-border text-foreground/80 flex items-center gap-2" onClick={() => setIsMenuOpen(false)}>
            <Phone size={20} className="text-green-600" />
            Call Now: {siteConfig.contactInfo.phone}
          </a>
